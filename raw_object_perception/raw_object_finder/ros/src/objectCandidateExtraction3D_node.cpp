...
std::string nodeName = "raw_object_finder";
...
raw_msgs::ObjectList finalClusteredObjectsMsg;
...
void objectCandidateExtractionCallback(const sensor_msgs::PointCloud2::ConstPtr& point_cloud_msg)
{
    ...
    ROS_INFO("Point cloud size: %i", augmentPointCloudCopy.points.size());
		...
		if (DO_CANDIDATION)
		{
			...
			for(unsigned int i=0; i < clusteredObjectsMsg.size(); ++i)
			{
				raw_msgs::Object obj;

				obj.cluster = clusteredObjectsMsg[i];
				obj.pose.pose.position = clusteredObjectsCentroidsMsg[i];
				object_list.objects.push_back(obj);
			}

			finalClusteredObjectsMsg = object_list;

			mutexExtractedObjects.unlock();

			pmd_pub4.publish(object_list);
		}
...
bool getKinectObjectCandidates3D(raw_srvs::GetObjects::Request &req, raw_srvs::GetObjects::Response &res)
{
	mutexExtractedObjects.lock();
	res.objects = finalClusteredObjectsMsg.objects;
	//res.pointCloudCentroids = finalClusteredObjectsCenroidsMsg;
	//res.bestPointCloudCentroidIndex = finalBestObjectsCentroidMsg;
	finalClusteredObjectsMsg.objects.clear();
...
int main(int argc, char **argv)
{
	...
	pmd_pub1 = n.advertise<sensor_msgs::PointCloud2> ("/raw_object_finder/kinect_objects", 1);
	pmd_pub2 = n.advertise<sensor_msgs::PointCloud2> ("/raw_object_finder/kinect_objects_surface", 1);
	pmd_pub3 = n.advertise<sensor_msgs::PointCloud2> ("/raw_object_finder/kinect_input_point_cloud", 1);
	pmd_pub4 = n.advertise<raw_msgs::ObjectList> ("/raw_object_finder/object_positions", 1);

	//Service Server
	srvGetKinectObjectCandidates = n.advertiseService("/raw_object_finder/get_objects", getKinectObjectCandidates3D);
	ros::ServiceServer srv_start = n.advertiseService("/raw_object_finder/start", start);
	ros::ServiceServer srv_stop = n.advertiseService("/raw_object_finder/stop", stop);
